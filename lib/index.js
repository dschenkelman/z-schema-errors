...
function createMessageReporter(options){
  ...
  function formatMessage(format, error, inner){
    return ['path', 'description', 'message', 'params', 'inner'].reduce(function(current, part){
      var partPlaceholder = '{' + part + '}';
      var output =
        part === 'inner' ?
          inner :
          error[part] ? extractors[part](error[part]) : '';
  ...
  function extractMessage(report) {
    return distinct(report.errors.map(function(error){
      ...
      var inner = '';
      if (error.inner){
        inner = extractMessage({ errors: error.inner });
      }
      return formatMessage(formats[error.code] || formats.DEFAULT, error, inner);
    })).join(' (also) ');
  }
